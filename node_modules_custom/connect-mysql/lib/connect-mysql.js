...
module.exports = function (connect) {
    ...
    MySQLStore.prototype.get = function (sid, callback) {
        ...
        let self = this;
        this.query(function(connection, done) {
            connection.query('SELECT `session` FROM `' + TableName + '` WHERE `sid` = :sid', {sid}, function (err, result) {
    ...
    MySQLStore.prototype.set = function (sid, session, callback) {
        ...
        this.query(function(connection, done) {
            connection.query('INSERT INTO `' + TableName + '` (`sid`, `session`, `expires`) VALUES(:sid, :session, :expires) ON DUPLICATE KEY UPDATE `session` = VALUES(`session`), `expires` = VALUES(`expires`)', {sid,session,expires}, function (err) {
                done(err);
            });
        }, callback);
    };
    ...
    MySQLStore.prototype.destroy = function (sid, callback) {
        this.query(function(connection, done) {
            connection.query('DELETE FROM `' + TableName + '` WHERE `sid` = :sid', {sid}, function (err) {
                done(err);
            });
        }, callback);
    };

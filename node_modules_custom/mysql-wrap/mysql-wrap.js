...
var createMySQLWrap = function (connection, options) {
    ...
    var toArray = function(rows, flds) {
      if (typeof rows==='undefined') {
        return;
      }
      if (rows.length==0) {
        return {f:flds.map(function(fld) {
          return fld.name
        }),d:[]}
      }
      return {
        f:flds.map(function(fld) {
          return fld.name
        }),
        d:rows.map(function(row) {
          var tmp = [];
          flds.forEach(function(fld) {
            tmp.push(row[fld.name]);
          });
          return tmp;
        })
      }
    ...
    var getValueFromParams = function (valuesOrCallback, callbackOrNothing) {
        return _.isArray(valuesOrCallback) || _.isObject(valuesOrCallback) ? valuesOrCallback : {};
    };
    ...
    self.queryArray = function (sqlOrObject, valuesOrCallback, callbackOrNothing) {
        var statement = getStatementObject(sqlOrObject),
            values = getValueFromParams(valuesOrCallback, callbackOrNothing),
            callback = getCallBackFromParams(valuesOrCallback, callbackOrNothing),
            def = Q.defer(),
        ...
        getConnection(readOrWrite(), function (err, conn) {
            if(err) {
                ...
                conn.query(statement, values, function(err, rows, flds){
                    respond(def, callback, err, toArray(rows, flds));
                    if (!transactionConn && conn.release) {
                        conn.release();
                    }
                });
